...
// Quote Flow modal - multi-step
function QuoteFlow({ open, onClose, initialItems = [] }) {
  const [step, setStep] = useS2(0);
  const [items, setItems] = useS2(initialItems);
  const [periodo, setPeriodo] = useS2('mensal');
  const [inicio, setInicio] = useS2('');
  const [local, setLocal] = useS2('');
  const [setor, setSetor] = useS2('Mineração');
  const [nome, setNome] = useS2('');
  const [empresa, setEmpresa] = useS2('');
  const [email, setEmail] = useS2('');
  const [tel, setTel] = useS2('');
  const [obs, setObs] = useS2('');
        ...
        <div className="quote-shell" style={{border: 0, borderRadius: 8}}>
          <aside className="quote-sidebar">
            ...
            <div style={{position:'absolute', bottom: 32, left: 32, right: 32, fontSize: 12, color:'rgba(255,255,255,.5)', fontFamily: 'var(--font-mono)', lineHeight: 1.5}}>
              ...
              <a href="https://wa.me/553836723091" target="_blank" style={{color: '#25D366', fontWeight: 600, marginTop: 6, display:'inline-block'}}>WhatsApp 24/7 →</a>
            </div>
          </aside>
            ...
            {done ? (
              <div className="success-box" style={{margin: 'auto'}}>
                ...
                <div style={{display:'flex', gap: 10, justifyContent:'center', marginTop: 28}}>
                  <a className="btn btn-whats" href="https://wa.me/553836723091" target="_blank">
                    <Icon.whats width="16" height="16"/> Acelerar pelo WhatsApp
                  </a>
                ...
                <div style={{marginTop: 32, padding: 16, background: 'var(--bg-2)', borderRadius: 4, textAlign:'left'}}>
                  ...
                  <div className="mono" style={{fontSize: 14, fontWeight: 600}}>GPD-{Math.floor(100000 + Math.random()*900000)}</div>
                </div>
              </div>
            ) : step === 0 ? (
              <>
                ...
                <div style={{display:'flex', gap: 10}}>
                  ...
                  {step < steps.length - 1 ? (
                    <button className="btn btn-primary" disabled={!canNext()}
                      ...
                      Continuar <Icon.arrow width="14" height="14"/>
                    </button>
                  ) : (
                    <button className="btn btn-primary" onClick={submit}>
                      Enviar solicitação <Icon.arrow width="14" height="14"/>
                    </button>
                  )}
                </div>
              </div>
